...
クラウドワークスでの「LP作成」「HTML/CSS修正」「小規模Web制作」案件向けに整理したポートフォリオです。
...
2. このフォルダの公開用ファイルをリポジトリへ追加します。
3. GitHubの `Settings` から `Pages` を開きます。
4. `Deploy from a branch` を選び、`main` ブランチのルートを公開対象にします。
5. 発行されたURLでトップページと各デモのリンクを確認します。
